...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$out = "<table class=\"$table_class\" cellspacing=0 border='1' width='800' style='font-size:12px'>";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
			if ($row_numbers) {
				$out .= "\n\t\t<th>&nbsp</th>";
	...
	function _format_value($format,$num,$f) {
		...
		$parts = preg_split('/;/', $format, -1, PREG_SPLIT_NO_EMPTY);
		$pattern = $parts[0];
		// Negative pattern
