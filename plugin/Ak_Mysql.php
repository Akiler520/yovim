...
class Ak_Mysql{
    ...
    public function setFieldCachePath($path){
        $this->cachePath = $path;
    }
    ...
    protected function cacheFields(){
        ...
        foreach($f as $key => $value){
            $fields[] = $value['Field'];
            if($value['Key'] == 'PRI'){
                $fields['_pk'] = $value['Field'];   // primary key
            }
            if($value['Extra'] == 'auto_increment'){
                $fields['_auto'] = $value['Field']; // auto increment field
            }
...
//        mysql_set_charset($this->charset);
    ...
    public function delete($data, $where=''){
        ...
        delete from table  order by  limit;
    ...
    public function getById($id, $fields = '*'){
        $sql = "SELECT $fields FROM ".$this->tabName." WHERE ".$this->fields['_pk']." = ".$id;
     ...
     * @return bool
     */
    function getRow($where, $fields = '*'){
        $sql = "SELECT $fields FROM ".$this->tabName." WHERE ".$where;
        $ret = $this->query($sql);
    ...
    public function count($where = '', $fields = ''){
        ...
        $this->sql="SELECT COUNT($fields) AS ab FROM ".$this->tabName.$where;
    ...
    public function __call($name,$param){
        if(strtolower(substr($name,0,5))=='getby'){
            ...
            $this->sql = "SELECT {$key} FROM {$this->tabName} " .
                    "WHERE {$fields}='{$value}'";
