 ...
 *	@package	LIB-Ak
 *
 *	$Id: Ak_Download.php 2013-02-19 akiler $
 */
class Ak_Download {
	/**
	 * if to be test
	...
	function downloadfile($filename, $downname='')
	{
		...
		if($this->filecheck())
		{
			...
			$downname = Ak_String::stripDiskFileName($downname);
			...
			if ($this->mineType == 'eml') {
				Header("Content-type: message/rfc822");		// use the code, can not active the 'auto select' checkbox.
			} else {
				Header("Content-type: ".$this->mineType);
			}
